...
```{r}
setwd("~/GitHub/UNIGE/CUSO_R/Cours_2")
#je charge les données que l'enseignant a préparé pour éviter les problèmes
#load("Cours_2.RData")
```

Pensez à lire ce que vous dit la console! Vous allez probablement devoir répondre "Oui" (ou "Yes") pour certaines installations
...
Le algorithme est assez compliqué, et le temps de calcul conséquent. On l'utilise peu avec des grandes bases de données (plus de 10 000 nœuds).
...
# Sauvegarde en html
rglfolder <- rglwidget(width=1000, height=1000)
htmlwidgets::saveWidget(rglfolder, "./graph.html")
...
```{r, include=TRUE}
write_graph(data, "data/edgelist.txt", format="edgelist")
...
visSave(data_3d_vis_plot,  file = "network.html", selfcontained = TRUE, background = "white")
```
